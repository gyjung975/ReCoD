...
def main(args):
    ...
    for idx, target_idx in enumerate(image_ids):
        ...
        for loop in range(args.loop):
            ...
            retrieved_image_caption = retrieval_store[retrieved_image_id]
            exclude_ids.append(retrieved_img_index)

            refer_caps = [retrieved_image_caption[ri] for ri in random.sample(list(range(5)), k=2)]
            args.query = args.query.replace('[txt]', '\n'.join(refer_caps))
            ...
            retri = {'id': int(retrieved_image_id),
                     'captions': retrieved_image_caption,
                     'used_captions': refer_caps,
...
def gen_infer_file(raw_file, args):
    if args.dataset == 'okvqa':
        ...
        for idx, l in enumerate(range(0, args.loop+1)):
            ...
            json.dump(question, open(f"diff/okvqa/claude_recod_{args.split}/claude_recod_{args.split}_total{l+1}.json", "w"), indent=4)
    else:
        ...
        for idx, l in enumerate(range(0, args.loop+1)):
            ...
            json.dump(aokvqa, open(f"diff/aokvqa/claude_recod_{args.split}/claude_recod_{args.split}_total{l+1}.json", "w"), indent=4)
...
if __name__ == '__main__':
    ...
    args.save_path = f'diff/{args.dataset}/claude_recod_{args.split}'
    args.save_name = f'claude_recod_{args.split}.json'
    print(os.path.join(args.save_path, args.save_name))

    os.makedirs(args.save_path, exist_ok=True)
    os.system("cp claude_recod.py %s/" % args.save_path)
    main(args)
