        ...
        for loop in range(args.loop):
            messages = [{"role": "user",
                         "content": [{"type": "image", "image": target_path[0],},
                                     {"type": "text", "text": "Describe the visual features of the image with great detail."},],}]
            ...
            generated_ids = model.generate(**inputs, max_new_tokens=40, temperature=temperature, top_p=top_p)
...
def gen_infer_file(raw_file, args):
    if args.dataset == 'okvqa':
        ...
        for idx, l in enumerate(range(0, args.loop)):
            ...
            if args.split == 'train': json.dump(question, open(f"diff/okvqa/qwen_train/qwen_train_final.json", "w"), indent=4)
        ...
        for idx, l in enumerate(range(0, args.loop)):
            ...
            if args.split == 'train': json.dump(aokvqa, open(f"diff/aokvqa/qwen_train/qwen_train_final.json", "w"), indent=4)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--init_new_tokens", type=int, default=40)
