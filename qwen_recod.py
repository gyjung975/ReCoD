...
def main(args):
    ...
    for idx in tqdm(range(0, len(image_ids), args.batch_size)):
        ...
        for loop in range(args.loop):
            ...
            messages = [{"role": "user",
                         "content": [{"type": "image", "image": target_path[0],},
                                     {"type": "text", "text": args.prompt.replace('[txt]', '\n'.join(refer_caps[0]))},],}]
...
def gen_infer_file(raw_file, args):
    if args.dataset == 'okvqa':
        ...
        for idx, l in enumerate(range(0, args.loop+1)):
            ...
            json.dump(question, open(f"diff/okvqa/qwen_recod_{args.split}/qwen_recod_{args.split}_total{l+1}.json", "w"), indent=4)
    else:
        ...
        for idx, l in enumerate(range(0, args.loop+1)):
            ...
            json.dump(aokvqa, open(f"diff/aokvqa/qwen_recod_{args.split}/qwen_recod_{args.split}_total{l+1}.json", "w"), indent=4)
...
if __name__ == '__main__':
    ...
    args.prompt = template.caption_based
    ...
    args.save_path = f'diff/{args.dataset}/qwen_recod_{args.split}'
    args.save_name = f'qwen_recod_{args.split}.json'
    print(os.path.join(args.save_path, args.save_name))

    os.makedirs(args.save_path, exist_ok=True)
    os.system("cp qwen_recod.py %s/" % args.save_path)
    main(args)
